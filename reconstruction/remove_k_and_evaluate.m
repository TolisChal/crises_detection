function [errors_stats, k_copulas_pairs] = remove_k_and_evaluate(Copulas, train_inds, n, cells, clr)

% This function trains a linear model to predict the mass of a copula's
...
%train the model
[Betas, Bias] = train_reconstruction_model(copulas_n_n, cells, 100/n, train_inds);
...
% Begin reconstruction and evaluation of the model
for i=1:n_eval
    cop = copulas_n_n{eval_inds(i)};
    %reconstruct copulas
    [esti_cop, error] = reconstruct_copula(cop, cells, Betas, Bias);
    errors(i,1) = error;
