Crisis_dates=[];
Events_dates=[];
Copulas_dates=[];
indicators=[];

ind_same = @(x) [1; find(diff(x))+1; length(x)]; % index for same areas

load('~/Dropbox/journal_cop/financial_crises/crisis_dates.mat','Crisis_dates');
load('~/Dropbox/journal_cop/financial_crises/events_dates.mat', 'Events_dates');
load('~/Dropbox/journal_cop/DJ600_Returns_top100/Copulas_uniform_dates.mat', 'Copulas_dates');
load('~/Dropbox/journal_cop/DJ600_Returns_top100/Copulas_uniform_indicators.mat', 'indicators');


Crisis_dates=Crisis_dates(Crisis_dates(:,1)>=Copulas_dates(1) & Crisis_dates(:,1)<=Copulas_dates(end),:);
Events_dates=Events_dates(Events_dates(:,1)>=Copulas_dates(1) & Events_dates(:,1)<=Copulas_dates(end),:);

idx=ind_same(ind_lbl);




figure('units', 'normalized','outerposition',[0 0 1 1]);

ax1=subplot(6,1,1);
hold on;
for i =1:size(Crisis_dates,1)
    fill([Crisis_dates(i,1) Crisis_dates(i,1) Crisis_dates(i,1)+60 Crisis_dates(i,1)+60],[0 6 6 0],'r')
end
plot(Copulas_dates,indicators)
datetick('x','yyyy')
set(gca,'XTickLabelRotation',45)
ylim([0,6])
title('Systematic Crisis (ECB FCDB 2017/07/31)')

ax2=subplot(6,1,2);
hold on;
for i =1:size(Events_dates,1)
    fill([Events_dates(i,1) Events_dates(i,1) Events_dates(i,1)+60 Events_dates(i,1)+60],[0 6 6 0],[0.9290, 0.6940, 0.1250])
end
plot(Copulas_dates,indicators, 'color', [0, 0.4470, 0.7410])
datetick('x','yyyy')
ylim([0,6])
set(gca,'XTickLabelRotation',45)
title('Residual Events (ECB FCDB 2017/07/31)')

ax=[subplot(6,1,3)];

hold on;
% for i =1:size(idx,1)-1
%     if ind_lbl(idx(i))==1
%         %         1
%         h=fill([Copulas_dates(idx(i)) Copulas_dates(idx(i)) Copulas_dates(idx(i+1)-1) Copulas_dates(idx(i+1)-1)],[0 6 6 0],'r');
%         set(h,'facealpha',.5)
%     elseif ind_lbl(idx(i))==2
%         %         2
%         h=fill([Copulas_dates(idx(i)) Copulas_dates(idx(i)) Copulas_dates(idx(i+1)-1) Copulas_dates(idx(i+1))],[0 6 6 0],[0.9290, 0.6940, 0.1250]);
%         set(h,'facealpha',.5)
%     end
% end

scatter(Copulas_dates,indicators,2,(indicators))
colormap(flipud(jet))
c = colorbar;
c.Position(1)=0.915;
ylbl=ylabel('Indicators');
ylbl.Rotation=0;
ylbl.Position(1)=-950;

set(gca,'XTickLabelRotation',45)
datetick('x','yyyy')
ylim([0,6])
title('Copulas Indicators')



for e_i=2:4
    ax=[ax subplot(6,1,e_i+2)];
    hold on;

    y_upper_lim=round(max(X2(:,e_i)),1);
    y_bottom_lim=round(min(X2(:,e_i)),1);
    
%     for i =1:size(idx,1)-1
%         if ind_lbl(idx(i))==1
%             %         1
%             h=fill([Copulas_dates(idx(i)) Copulas_dates(idx(i)) Copulas_dates(idx(i+1)-1) Copulas_dates(idx(i+1)-1)],[y_bottom_lim y_upper_lim y_upper_lim y_bottom_lim],'r');
%             set(h,'facealpha',.5)
%         elseif ind_lbl(idx(i))==2
%             %         2
%             h=fill([Copulas_dates(idx(i)) Copulas_dates(idx(i)) Copulas_dates(idx(i+1)-1) Copulas_dates(idx(i+1))],[y_bottom_lim y_upper_lim y_upper_lim y_bottom_lim],[0.9290, 0.6940, 0.1250]);
%             set(h,'facealpha',.5)
%         end
%     end
    
    scatter(Copulas_dates,X2(:,e_i),2,(indicators))
    
    colormap(flipud(jet))
    c=colorbar;
    
    c.Position(1)=0.915;
    
    datetick('x','yyyy')
    ylbl=ylabel(strcat('Eigenvector',{' '},int2str(e_i)));
    % set(get(gca,'YLabel'),'Rotation',0)
    ylbl.Rotation=0;
    ylbl.Position(1)=-950;
    ylim([y_bottom_lim y_upper_lim])
    if i==2
        title('Eigenvectors')
    end
    set(gca,'XTickLabelRotation',45)
end

linkaxes(ax,'x')
datetick('x','yyyy')